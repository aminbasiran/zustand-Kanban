...
const Column = ({state}) => {
    ...
    return (
        <div onDragOver={(e)=>e.preventDefault()} onDrop={()=> {moveTask(draggedItem.title,state,draggedItem.tags),
                                                                setDraggedItem(null)   
                                                            }} 
        className={`flex-grow ${tasks.length === 0 ? "border-4 border-dashed border-neutral-400/30" : "rounded-lg bg-zinc-900" }  min-h-64 p-3`}>
            ...
            <div className='flex flex-col gap-2 '>
                {tasks.length === 0 ? <h2 className='text-lg '>Empty list</h2> : tasks.map(task => <Task key={task.title}  title={task.title} state={task.state} tags={task.tags}/>)}
            </div>
